...
2. TypeScript dosyalarını derlemek için npm install ve ardından npm run build (veya npx tsc) komutlarını çalıştırın (kaynaktan derliyorsanız).
3. Chromium tabanlı (Chrome, Brave, Opera) tarayıcınızı açın.
4. Adres çubuğuna `chrome://extensions/` yazın.
5. Sağ üstteki **Geliştirici modunu** (Developer mode) aktif edin.
6. **Paketlenmemiş öğe yükle** (Load unpacked) butonuna tıklayın ve indirmiş olduğunuz klasörü seçin.
7. Eklenti simgesi sağ üst köşede belirecektir!
